...
describe('StaticSmith', function(){
  beforeEach(function(){
    rm('test/tmp');
  });

  it('should expose a constructor', function(){
    assert.equal(typeof StaticSmith, 'function');
  });

  it('should not require the `new` keyword', function(){
    var m = StaticSmith('test/tmp');
    assert(m instanceof StaticSmith);
  });
  ...
  describe('#read', function(){
    it('should read from a source directory', function(done){
      var m = StaticSmith(fixture('read'));
    ...
    it('should read from a provided directory', function(done){
      var m = StaticSmith(fixture('read-dir'));
    ...
    it('should preserve an existing file mode', function(done){
      var m = StaticSmith(fixture('read-mode'));
    ...
    it('should expose the stats property in each file metadata', function(done){
      var m = StaticSmith(fixture('expose-stat'));
      m.read(function(err, files) {
    ...
    it('should not parse frontmatter if frontmatter is false', function(done){
      var m = StaticSmith(fixture('read-frontmatter'));
      m.frontmatter(false);
      m.read(function(err, files){
        if (err) return done(err);
    ...
    it('should still read all when concurrency is set', function(done){
      var m = StaticSmith('test/fixtures/concurrency');
      m.concurrency(3);
      m.read(function(err, files){
        if (err) return done(err);
    ...
    it('should ignore the files specified in ignores', function(done){
      ...
      var m = StaticSmith('test/fixtures/basic');
      m.ignore('nested');
      m.read(function(err, files){
        if (err) return done(err);
        assert.deepEqual(files, {
  ...
  describe('#write', function(){
    it('should write to a destination directory', function(done){
      var m = StaticSmith(fixture('write'));
    ...
    it('should write to a provided directory', function(done){
      var m = StaticSmith(fixture('write-dir'));
      var files = { 'index.md': { contents: new Buffer('body') }};
    ...
    it('should chmod an optional mode from file metadata', function(done){
      var m = StaticSmith(fixture('write-mode'));
      var files = {
        'bin': {
          contents: new Buffer('echo test'),
          mode: '0777'
  ...
  describe('#run', function(){
    it('should apply a plugin', function(done){
      var m = StaticSmith('test/tmp');
      m.use(plugin);
      m.run({ one: 'one' }, function(err, files, staticsmith){
        assert.equal(files.one, 'one');
        assert.equal(files.two, 'two');
        done();
      });

      function plugin(files, staticsmith, done){
        assert.equal(files.one, 'one');
        assert.equal(m, staticsmith);
    ...
    it('should run with a provided plugin', function(done){
      var m = StaticSmith('test/tmp');
      m.run({ one: 'one' }, [plugin], function(err, files, staticsmith){
        assert.equal(files.one, 'one');
        assert.equal(files.two, 'two');
        done();
      });
    ...
    it('should support synchronous plugins', function(done){
      ...
      function plugin(files, staticsmith){
        assert.equal(files.one, 'one');
        assert.equal(m, staticsmith);
        files.two = 'two';
      }
  ...
  describe('#build', function(){
    ...
    it('should preserve binary files', function(done){
      StaticSmith(fixture('basic-images'))
        .build(function(err, files){
          if (err) return done(err);
          assert.equal(typeof files, 'object');
    ...
    it('should apply a plugin', function(done){
      StaticSmith(fixture('basic-plugin'))
        .use(function(files, staticsmith, done){
    ...
    it('should remove an existing destination directory', function(done){
      var m = StaticSmith(fixture('build'));
    ...
    it('should not remove existing destination directory if clean is false', function(done){
      var m = StaticSmith(fixture('build-noclean'));
      m.clean(false);
...
describe('CLI', function(){
  var bin = path.resolve(__dirname, '../bin/staticsmith');

  describe('build', function(){
    it('should error without a staticsmith.json', function(done){
      exec(bin, { cwd: fixture('cli-no-config') }, function(err, stdout){
        assert(err);
        assert(~err.message.indexOf('could not find a staticsmith.json configuration file.'));
        done();
      });
    });

    it('should grab config from staticsmith.json', function(done){
    ...
    it('should error when failing to require a plugin', function(done){
      exec(bin, { cwd: fixture('cli-no-plugin') }, function(err){
        assert(err);
        assert(~err.message.indexOf('failed to require plugin "staticsmith-non-existant".'));
        done();
      });
    });
